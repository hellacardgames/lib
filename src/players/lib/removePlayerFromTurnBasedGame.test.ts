...
import { removePlayerFromTurnBasedGame } from "./removePlayerFromTurnBasedGame.js";
...
test("removes the given player", () => {
  ...
  let result = removePlayerFromTurnBasedGame(game, "player-id-002");
  game = result.game;
  expect(game.players).toEqual([player1, player3]);

  result = removePlayerFromTurnBasedGame(game, "player-id-003");
  game = result.game;
  expect(game.players).toEqual([player1]);

  result = removePlayerFromTurnBasedGame(game, "player-id-001");
  game = result.game;
...
test("keeps current player when an earlier player is removed", () => {
  ...
  const result = removePlayerFromTurnBasedGame(game, "player-id-002");
  game = result.game;
...
test("keeps current player when a later player is removed", () => {
  ...
  const result = removePlayerFromTurnBasedGame(game, "player-id-003");
  game = result.game;
...
test("changes turn when removing current player", () => {
  ...
  const result = removePlayerFromTurnBasedGame(game, "player-id-001");
  game = result.game;
...
test("throws when player does not exist in game", () => {
  ...
  expect(() => removePlayerFromTurnBasedGame(game, "some-random-id")).toThrow(
